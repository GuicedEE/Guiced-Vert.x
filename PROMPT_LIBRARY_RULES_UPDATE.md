...
- Library name: Guiced Vert.x Client (GuicedEE ↔ Vert.x bridge)
- Current/new version: <VERSION>
- Repository URL / path: https://github.com/GuicedEE/Guiced-Vert.x.git
- Short description: Bridge client between GuicedEE and Vert.x; registers Vert.x via GuicedEE client, offers utilities/config annotations/SPIs, and exposes Vertx injection bound as a static singleton during GuicedEE Client PreStartup.
- Rules Repository placement for this library’s rules: rules/generative/backend/guicedee/vertx (register and publish the modular rules here).
- Type:
...
- Component/topic areas (list): Vert.x integration via GuicedEE client; Vertx injection binding; configuration annotations; utility helpers; SPI hooks for extension.
...
Perform as a single, forward-only change set. The exact target paths depend on your library repo structure (e.g., docs/, guides/, or packages/<lib>/docs/). Default policy: host library docs live in the library repo (not inside a Rules Repository submodule) unless the task explicitly targets the Rules Repository location below.

For this library, publish and register the Rules Repository content under rules/generative/backend/guicedee/vertx (use this path for the modular rules/index files).
